...
- Commonly used statistical tests make assumptions
  ...
  - Data are most often not normally distributed (errors)
...
- Best estimate of sd of sampling distribution of means is the standard error of mean
...
rts_wide %>%
  group_by(name) %>%
  summarise(mean=mean(value),  trim_20=mean(value, trim=.2))
...
rts_wide %>%
  group_by(name) %>%
  summarise(win=datawizard::winsorize(value), mean=mean(win)) %>% 
kableExtra::kable()
```
---
...
- SE, CI, and p-values constructed from the data that researcher collected... using a computer algorithm!  
  ...
  - No mathematical formula / equations involved--kinda
---
#Bootstrap: Big idea
...
$$\hat{z}_0 = \Phi^{-1}\left( \frac{1}{R+1} \sum_{r = 0}^R I( \hat{\theta}{}_r^* < \hat{\theta})  \right) \qquad $$

```{r, eval=FALSE}
#$$\phi^-1 (.975)= #$$\hat \theta_{r}$$  = Mean of bootstraps#$$\hat \theta$$  = Mean of sample#$$ B $$ = # of bootstraps
theat.hat <- mean(x)
...
3. Minimizes the influence of outliers without trivializing the inferential value of them (when using robust estimators like bca)
...
- Not very good if error is not constant
  ...
  - Linear modeling assumes equal variances 
]
...
$$\boldsymbol{\sigma^2_B} = \begin{bmatrix}\mathrm{Var}(b_0) & \mathrm{Cov}(b_0,b_1) \\ \mathrm{Cov}(b_0,b_1)  & \mathrm{Var}(b_1) \end{bmatrix}$$
  ...
  - Robust SEs work by down-weighting large residuals
  
- Many types

  -**HC3** sandwich estimator 

$$HC3: \frac{\hat{\mu}_i^2}{(1 – h_i)^2}$$
```{r, eval=FALSE}
hc3 <- resid(model)^2/(1 - hatvalues(model))^2
```

.footnote[.small[Find more information on SEs here:https://theeffectbook.net/ch-StatisticalAdjustment.html#your-standard-errors-are-probably-wrong]]
---
# Getting Robust SEs in R 

- `Easystats` `model_paramters`
```{r}
