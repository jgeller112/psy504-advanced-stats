...
#| warning: false
#| 
master <- read_csv("/Users/jasongeller/Documents/Mediation/data/mediation.csv")
...
-   Previous study time negatively impacts facebook time (a path = -.21)

-   Controlling for Facebook time, previous experience positively impacts exam scores (b path = .26)

-   Controlling for previous studying, Facebook time positively impacts exam scores (c' path = -0.61)
...
-   Assumes indirect effect is normally distributed
    -   It is not!
        ...
        -   It is slightly skewed and leptokurtic
...
med_results$boot.ci # only normal # need bca
```
...
mediate_plot(exam~facebook + previous,data=master)
...
#| fig.width=8
#| fig.height=4
library(mma) # get dataset
data("weight_behavior")
weight_behavior <- drop_na(weight_behavior) 
```
...
-   State your hypothesis and your statistical test

    -   How indirect effect was tested and number of bootstraps
...
## Example write-up: Simple mediation

The mediation analysis was conducted using the R package memobootR (Buchanon) with a bootstrapping procedure to estimate the indirect effect of Facebook usable on exam scores through studying time. The model included facebook usage as the predictor, exam score as the outcome, and study time as the mediator. The significance of the indirect effect was tested using bias-corrected bootstrap confidence intervals (bca CIs) based on 10,000 bootstrap samples.

## Example write-up

The results indicate a significant total effect between facebook use and exam scores (b = -0.661, SE = 0.128, t(236) = 5.16, p \< .001). There was also a significant direct effect (c') on facebook use and exam score while holding studying time constant (b = -0.606, SE = 0.127, t(237) = 4.77, p \< .001). The a path (b = -.21, SE = 0.097, t(237) = 2.17, p = .031) and b (b = 0.26, SE = 0.084, t(237) = 3.09, p \< .001) paths were both significant. The indirect effect was also significant (indirect effect = -0.05, 95% CI \[-0.18, -0.01\]) indicating that previous knowledge/studying mediated the relationship between Facebook use and exam scores.

## Example write-up: Multiple mediators
