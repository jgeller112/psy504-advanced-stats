...
4.Missing data patterns with `md.pattern()`

-   Use the `mice::md.pattern()` function to do a missing data pattern analysis. Adjust the code from lecture to do a similar analysis with variables `iqv`, `ses`, `lpr` and `lpo`. Run the code both ways, with `plot = FALSE` and `plot = TRUE`.
...
Impute with the `d` data set and save the results as
...
We will come back to these models at the end.

## 10. Regression impute

-   Impute the missing data with a regression approach

-   Save the data as `regression_lm`
...
1.  Use the `str()` function to get an overview of the contents in your `imp1` object.

2.  You may have noticed there's a lot of stuff in there. Let's take a more focused look by executing `imp1$method`.

3.  We didn't emphasize this in class, but there are several ways to do multiple imputation. By default, the `mice()` function used the *predictive mean matching* method, which is the same as setting `method = "ppm"`

4.  Next execute `imp1$predictorMatrix`. van Buuren explained how to interpret this in Section 6.3.2.

5.  Next execute `imp1$formulas`. These are the linear regression formulas `mice()` used to impute the missing values.

6.  With the last two bullets in mind, notice how we used the `pup` variable in the imputation formulas. Since `pup` is just a participant id number, this is a really bad idea. We're injecting random noise into our data.

7.  Also notice that we used all four `r` dummy variables in the imputation formulas and we even had imputation formulas for the dummy variables themselves. This is also a really bad idea. I know the definition of what a variable is is getting a little hazy, here. But hopefully you are building an intuition that the four `r` dummy variables are different in nature from the original variables they were based on.

8.  Execute `imp1$loggedEvents`. van Buuren covered how to interpret this in Section 6.3.2. In short, our four `r` dummies caused problems and the nature of those problems is displayed in this output. Don't go too far down the rabbit hole on this one. Just notice that we have problems.
...
-   Also, to make the next exercise easier, set `m = 4`, which will result in 4 imputed data sets.
...
-   The plots above are a little unsatisfying because there's no distinction between the observed and imputed values. Fortunately, we can use our `r_lpr` and `r_lpo` dummies to tell which cases had missingness on `lpr` and/or `lpo`. The code below will give you the row numbers for all those cases. Execute that code.

**Tip**: Play around with the code I gave you and make sure you understand what it's doing.

```{r}
missing_in_lpr_or_lpo <- d %>% 
  mutate(.id = 1:n()) %>% 
  filter(r_lpr == 0 | r_lpo == 0) %>% 
  pull(.id)
```

-   Now execute the following code to make a subset of your `d_imp` data, which we'll name `d_imp_small`.

```{r}
d_imp_small <- d_imp %>% 
  filter(.id %in% missing_in_lpr_or_lpo)
```

-   Now make another faceted scatter plot, just like in task #10, but this time with the `d_imp_small` data. If you are successful, you will get a scatter plot that only shows the imputed values.
