...
```
---
# Permutation: Calculate Test Statistic on Permuted Data
...
perm.diff <- one.perm %>%
  ...
  summarise(diff_group = diff(mean))
...
---
# CALCULATE THE TEST STATISTIC (DIFFERENCE IN PERMUTED GROUP MEANS) 
...
many.perm.diffs_1 <- many.perm.diffs %>% 
  mutate(abs_obs_dif = abs(pull(mean_diff)), # diff from our sample
         abs_perm_dif = abs(diff_value), # permutations diffs
         as_or_more_extreme = abs_perm_dif >= abs_obs_dif) # count how many
